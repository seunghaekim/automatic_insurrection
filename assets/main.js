...
const generated = () => {
    ...
    const pull_quote = sentences[0]
    const shuffled_sentences = array_shuffle(sentences)
    const babble_arr = shuffled_sentences.slice(0, 4)
    const babble = babble_arr.join(' ')
    const more_babble_arr = shuffled_sentences.slice(4, 8)
    const more_babble = more_babble_arr.join(' ')
    const even_more_babble_arr = shuffled_sentences.slice(8, shuffled_sentences.length)
    const even_more_babble = even_more_babble_arr.join(' ')
    return {
        title: title(),
        sentences,
        pull_quote,
